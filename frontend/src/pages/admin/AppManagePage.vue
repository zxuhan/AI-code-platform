      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'action'">
          <a-space>
            ...
            <a-popconfirm title="Are you sure you want to delete this application?" @confirm="deleteApp(record.id)">
...
const toggleFeatured = async (app: API.AppVO) => {
  ...
  try {
    ...
    if (res.data.code === 0) {
      message.success(newPriority === 99 ? 'Marked as featured' : 'Featured flag removed')
      // Refresh data
      fetchData()
    } else {
